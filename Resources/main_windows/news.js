  ...
  activity.onCreateOptionsMenu = function(e) {
    ...
    menuItem.addEventListener("click", function(e) {
      if (Ti.Network.online) {
        ...
        var tweetxhr = Ti.Network.createHTTPClient();
        tweetxhr.timeout = 1000000;
        tweetxhr.open("GET","http://api.twitter.com/1/statuses/user_timeline.json?screen_name=slconference");
        tweetxhr.onload = updateTweets;
        tweetxhr.send();
      } else {
...
function updateTweets() {
  try {
    ...
    // Store so we can check for new ones later
    Titanium.App.Properties.setString("last_tweet", tweets[0].created_at);
    for (var c=0;c<tweets.length;c++){
		...
		tableview = Titanium.UI.createTableView({data:data,minRowHeight:58});
		if (Ti.Platform.name == "android") {
      tableview.minRowHeight = 100;
    }
		
		win.add(tableview);
  } 
  catch(E) {
    alert("Sorry, but I could not get latest tweets from @slconference.");
  }
}

// Add in the refresh button
...
refresh.addEventListener('click', function(e) {
  if (Ti.Network.online) {
    ...
    var alertDialog = Ti.UI.createAlertDialog({
      title: "",
      message: "You must be online in order to access our updates from @slconference.",
      buttonNames: ['OK']
    });
    alertDialog.show();
  }
...
// Add in an auto refresh on window focus
...
xhr.timeout = 1000000;
xhr.open("GET","http://api.twitter.com/1/statuses/user_timeline.json?screen_name=slconference");

xhr.onload = updateTweets;
xhr.send();